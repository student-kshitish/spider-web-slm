# train_final.py
# Final clean run: dim=64, Variant-A SRM (broadcast write), fp32 master + autocast(bf16)
# 30000 steps from step 0, checkpoints/final_run/
...
HARD_ROUTING   = False   # never switch to hard routing

CKPT_DIR = Path("checkpoints/final_run")
CKPT_DIR.mkdir(parents=True, exist_ok=True)

LAST_CKPT = CKPT_DIR / "last.pt"
BEST_CKPT = CKPT_DIR / "best.pt"

LOG_FILE = Path("logs/final_run.csv")
...
def train():
    ...
    print(f"  steps=30000  fresh from step 0")
    print(f"  save → {CKPT_DIR}/\n")

    # --------------------------------------------------------
    # MODEL  (float32 master weights)
    # --------------------------------------------------------
    ...
    # Reset iterator so training starts from the beginning of the data
    train_iter = iter(train_loader)
    ...
    prev_ema        = None
    consec_increase = 0
    ce              = init_ce   # fallback for final print if loop is empty
